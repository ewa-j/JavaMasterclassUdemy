...
//If all parameters are valid, the method needs to calculate the number of buckets and return it.
  ...
  public int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
    ...
    double areaCoveredByExtraBuckets = 0;
    if (extraBuckets > 0) {
      areaCoveredByExtraBuckets = areaPerBucket * extraBuckets;
    }
    return (int) Math.ceil((area - areaCoveredByExtraBuckets)/areaPerBucket);
  }
